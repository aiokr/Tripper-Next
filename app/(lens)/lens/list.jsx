      ...
      <div key={data.no} className='mb-8'>
        <Link href={`/lens/${data.no}`} id={`${data.no}`}>
          <div className={`${style['lensPhoto']} max-w-full`}>
            <Image src={data.image} className='object-cover max-w-[100%] max-h-[800px]' placeholder='blur' blurDataURL='data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mMsW3LkPwAGVQLf/ytPgwAAAABJRU5ErkJggg==' width={1200} height={800} alt={data.title} unoptimized />
          </div>
        </Link>
        <div className={`${style['lensPhotoInfo']} mt-4 flex flex-row gap-8 justify-center items-center opacity-60 text-sm overflow-x-auto`} style={{
